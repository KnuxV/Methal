...
SINDIV=$PTHINDIV/work/plays/jost-01-daa-im-narrehuss
SGRP=$PTHGRP/work/plays/jost-daa-im-narrehuss
...
for dname in $SINDIV $SGRP; do
    ...
    for sdname in out utils data; do
      [[ ! -d $dname/$sdname ]] && mkdir $dname/$sdname
    done
    #cp -r $DATA $dname/.
    cp -r $LOGS $dname/.
    #cp -r $UTILS $dname/.
    ...
    for xname in $(ls $OUT); do
      if [[ -z $(echo "$OUT/$xname" | grep -P "(bak|copy|test|dracor)") ]]; then
    ...
    for fname in $CREATE_TEIINITIAL $CREATE_TEIFINAL $CONFIG $OCONFIG $FRONT $VERSE $PP; do
...
TEIBASIC=../out/e_daa_im_narrehuss.xml
CREATE_TEIFINAL=../out/e_daa_im_narrehuss_with_characters.xml
