...
def manual_cleanups(st):
    ...
    st = re.sub(re.compile(r"""<sp>\s*<speaker>ADDES - EDDES \(pfeifen die Melodie:</speaker>\s*<p>« D’r Hans im Schnokeloch »</p>\s*</sp>\s*<stage>und schlagen im Takt auf den Klopfstein\).</stage>""",
                re.DOTALL),
                "<stage>ADDES - EDDES (pfeifen die Melodie: « D’r Hans im Schnokeloch » und schlagen im Takt auf den Klopfstein).</stage>",
                st)
    st = re.sub(re.compile("""</p>\s*</sp>\s*<sp>\s*<speaker>läwe, drej Käs hoch, vive!</speaker>""",
                re.DOTALL),
                """läwe, drej Käs hoch, vive!</p>""",
                st)
    #st = re.sub(re.compile("""<sp>\s*<speaker>(6. UFFTRITT) (Drohtspitz, 50jährig, stupfelbärtig, den Hut auf einem Ohr, grosse Zigarre im Mund, bleibt an der Mitteltüre stehn.) (DROHTSPITZ:)</speaker>"""),
    #            r"</div><div type='scene'><head>\1</head><stage>\2</stage><sp><speaker>\3</speaker>", st)
    #breakpoint()
    #st = st.replace("<stage/>", "")
    #st = st.replace("<stage>16</stage>", "")
    #st = re.sub(
    #    r"""(D’ Madam Drohtspitz hett’s)</p>\s*</sp>\s*(?:<stage/>\s*)?<sp>\s*<speaker>""",
    #    r"\1", st)
    # st = re.sub(re.compile(r"""</p>\s*</sp>(<speaker>)(nimi notwendig Gäns ze stopfe, diss isch guet for Proletarier.) """, re.DOTALL),
    #             r"\2 \1", st)
    #st = re.sub(r"""<stage>16</stage>\s*<sp>\s*<speaker>""", "", st)
...
def create_xml(xml_list):
    ...
    body_text = body_text.replace("""</p></sp> <stage></stage> nimi notwendig Gäns ze stopfe, diss isch guet for Proletarier. ADDES:""",
                                  "nimi notwendig Gäns ze stopfe, diss isch guet for Proletarier.</p></sp><sp><speaker>ADDESXXX")
    ...
    # ser =  re.sub(re.compile(r"""</p>\s*</sp>(<speaker>)(nimi notwendig Gäns ze stopfe, diss isch guet for Proletarier.) """.encode(), re.DOTALL),
    #               rb"\2 \1", ser)
    ser = ser.replace('<speaker>nimi notwendig Gäns ze stopfe, diss isch guet for Proletarier. ADDES:'.encode(),
                      'nimi notwendig Gäns ze stopfe, diss isch guet for Proletarier. <speaker>ADDES:'.encode())
