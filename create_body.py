...
def create_body(path, startline):
    ...
    def handle_table(wrapper, sp_class):
        ...
        for index, table_line in enumerate(wrapper):
            ...
            # Regex
            match_single_char_table = re.search(re_single_char_table, table_line)
            ...
            if re.match(re_table_close, table_line):
                ...
                if match_single_char_table:
                    ...
                    who_table = "#" + match_single_char_table.group("char").lower().replace(" ", "_")
                    sp.add_speaker(speaker_table, who_table)
                elif match_line_table:
                    if not sp.has_lg:
                        lg = Lg(sp.xml_sp)
                        sp.change_lg()
                    lg.add_poem(match_line_table.group("line"), match_line_table.group("part"))
    ...
    body = Body()
    poem_bool = False
    # To deal with a problem of <li> between two table and without a <small> bla <small> which acts as speaker
    end_of_table = False
    # end of G'sand // To deal with a problem where there's no <poem> before G'sand
    g_sang = False
    with open(path, 'r', encoding="utf-8") as f:
        ...
        for index, line in enumerate(f):
            ...
            if match_act:
                ...
                who = "#" + match_single_char.group("char").lower().replace(" ", "_")
...
if __name__ == "__main__":
    create_body("data/txt/Babette,_mach_'s_Fenster_züe.txt", 25)
